class PlayersController < ApplicationController
  ...
  def index
    @position_exclude_list = ['DB', 'LB', 'DL']
    @players = Player.where.not(position: @position_exclude_list)
    if params[:query].present?
      @players = @players.search(params[:query], page: params[:page])
    else
      @players = @players.all.page params[:page]
    end
  end
